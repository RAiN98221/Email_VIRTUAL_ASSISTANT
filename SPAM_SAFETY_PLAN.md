...
- Track suppression reasons such as `unsubscribed`, `bounced`, `blocked`, `complained`, and `replied_stop`.
...
Status: implemented for the completed safety features.
...
1. Done: queue guardrails with daily cap and interval jitter.
2. Done: suppression table, preview filtering, and manual suppress/unsuppress controls.
3. Done: conservative hard SMTP failure classification and auto-pause.
4. Done: unsubscribe token, endpoint, email footer, and headers.
5. Partial: job cards show key safety state; a dedicated safety summary endpoint can still be added later.
6. Done: unit tests and non-destructive runtime smoke checks.

## Deferred

- Inbox reply parsing is deferred. Automatically detecting "stop" or unsubscribe intent from replies needs inbox polling and review logic to avoid false positives.
